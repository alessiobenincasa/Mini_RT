...
/*   Updated: 2024/03/29 17:06:11 by svolodin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//     t_ray t_ray = transform(ray, inverse(cyl->transform));
//     t_tuple cyl_to_ray = subtract_tuples(t_ray.origin, cyl->center);
    
//     double a = pow(t_ray.direction.x, 2) + pow(t_ray.direction.z, 2);
//     if (fabs(a) > EPSILON)
//     {
//         double b = 2 * t_ray.origin.x * t_ray.direction.x + 2 * t_ray.origin.z * t_ray.direction.z;
//         double c = pow(t_ray.origin.x, 2) + pow(t_ray.origin.z, 2) - 1;
...
//             double y0 = t_ray.origin.y + t0 * t_ray.direction.y;
//             if (y0 > cyl->minimum && y0 < cyl->maximum)
//                 add_intersection_cylinder(&xs, t0, cyl);

//             double y1 = t_ray.origin.y + t1 * t_ray.direction.y;
...
//     if (cyl->closed && fabs(t_ray.direction.y) > EPSILON)
//     {
//         double t_lower = (cyl->minimum - t_ray.origin.y) / t_ray.direction.y;
//         if (check_cap(t_ray, t_lower, cyl->diameter))
//             add_intersection_cylinder(&xs, t_lower, cyl);
//         double t_upper = (cyl->maximum - t_ray.origin.y) / t_ray.direction.y;
//         if (check_cap(t_ray, t_upper, cyl->diameter))
...
t_intersections local_intersect_cylinder(t_cylinder *cyl, t_ray ray)
{
    ...
    if (fabs(a) > EPSILON) {
        ...
        double c = pow(tr_ray.origin.x, 2) + pow(tr_ray.origin.z, 2) - 1;
        double disc = b * b - 4 * a * c;
